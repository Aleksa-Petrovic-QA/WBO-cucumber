...
class PenLines {
	...
	@And("Verify two lines are displayed")
	def verifyLines() {
		WebUI.takeScreenshotAsCheckpoint('penLines', [])
	}
	
	@When("User increaces pen size")
	def increacePenSize() {
		WebUI.callTestCase(findTestCase('WBO/Actions/Increase pen size'), [:], FailureHandling.STOP_ON_FAILURE)
	}
	
	@And("Whites out one line")
	def whiteOutLine() {
		WebUI.click(findTestObject('Sidebar menu/Pencil icon'))
